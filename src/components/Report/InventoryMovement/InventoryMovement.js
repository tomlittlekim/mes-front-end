...
import { inventoryMovementColumns, inventoryMovementGridProps } from './InventoryMovementReport';
...
const InventoryMovement = (props) => {
  ...
  const columns = inventoryMovementColumns;
  const gridProps = inventoryMovementGridProps;
      ...
      <Grid container spacing={2} mt={0}>
        <Grid item xs={12} md={6}>
          <Box sx={{ height: 400, bgcolor: getBgColor(), borderRadius: 1, p: 2, border: `1px solid ${getBorderColor()}` }}>
            <Typography variant="h6" sx={{ mb: 2, color: getTextColor() }}>품목별 입출고량</Typography>
